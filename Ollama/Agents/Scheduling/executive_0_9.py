...
class executive:
    ...
    # ====== Local Calendar Tools (FIXED) ======
    
    def _load_local_calendar(self):
        """
        Helper method to load the local calendar file.
        Handles multiple calendars in one file using parse_multiple().
        Returns the first calendar or creates a new one if none exists.
        """
        if not LOCAL_CALENDAR_FILE.exists():
            logger.info("[Local Calendar] File does not exist, creating new calendar")
            return Calendar()
        ...
        try:
            ...
            if not content.strip():
                logger.info("[Local Calendar] File is empty, creating new calendar")
                return Calendar()
            
            # Use parse_multiple to handle files with multiple calendars
            calendars = list(Calendar.parse_multiple(content))
            
            if calendars:
                logger.info(f"[Local Calendar] Loaded calendar with {len(calendars)} calendar(s)")
                return calendars[0]  # Use the first calendar
            else:
                logger.info("[Local Calendar] No calendars found in file, creating new calendar")
                return Calendar()
                
        except Exception as e:
            logger.error(f"[Local Calendar] Error reading calendar file: {e}")
            logger.warning("[Local Calendar] Creating new calendar due to read error")
            return Calendar()
    
    def _save_local_calendar(self, cal):
        """
        Helper method to save the local calendar file.
        """
        try:
            with open(LOCAL_CALENDAR_FILE, "w") as f:
                f.writelines(cal.serialize_iter())
            logger.info("[Local Calendar] Calendar saved successfully")
            return True
        except Exception as e:
            logger.error(f"[Local Calendar] Error writing calendar file: {e}")
            raise

    def add_event_local(self, summary, start_time, end_time):
        """Add an event to the local calendar file"""
        try:
            cal = self._load_local_calendar()
            ...
            self._save_local_calendar(cal)
            logger.info(f"[Local Calendar] Event '{summary}' added successfully")
            return f"Event '{summary}' added to local calendar."
            
        except Exception as e:
            logger.error(f"[Local Calendar] Error adding event: {e}")
            raise
    ...
    def add_recurring_event_local(self, summary, start_time, end_time, recurrence_rule):
        ...
        try:
            ...
            self._save_local_calendar(cal)
            logger.info(f"[Local Calendar] Recurring event '{summary}' added successfully")
            return f"Recurring event '{summary}' added to local calendar with rule '{recurrence_rule}'."
            
        except Exception as e:
            logger.error(f"[Local Calendar] Error adding recurring event: {e}")
            raise

    def delete_event_local(self, event_name):
        """Delete an event from the local calendar by name"""
        if not LOCAL_CALENDAR_FILE.exists():
        ...
        try:
            ...
            event_found = False
            for event in list(cal.events):  # Create a copy of the set to iterate over
            ...
            if event_found:
                self._save_local_calendar(cal)
                logger.info(f"[Local Calendar] Event '{event_name}' deleted successfully")
                return f"Event '{event_name}' deleted from local calendar."
            else:
                logger.warning(f"[Local Calendar] Event '{event_name}' not found")
                return f"Event '{event_name}' not found in local calendar."
                
        except Exception as e:
            logger.error(f"[Local Calendar] Error deleting event: {e}")
            raise
            
    def get_events_local(self, days_ahead=7):
        """Get upcoming events from the local calendar"""
        if not LOCAL_CALENDAR_FILE.exists():
            logger.info("[Local Calendar] Calendar file does not exist")
            return []
        ...
        try:
            ...
            for e in cal.events:
                ...
                try:
                    ...
                    logger.warning(f"[Local Calendar] Skipping event {e} due to error: {ex}")
                    continue

            logger.info(f"[Local Calendar] Found {len(events)} events in range")
            return events
            
        except Exception as e:
            logger.error(f"[Local Calendar] Error getting events: {e}")
            return []
