...
class Citation:
    ...
    def to_dict(self):
        ...
        d.pop("full_text", None)   # don't send full text over WS
        return d
...
DEFAULT_INSTANCES: list[OllamaInstance] = [
    OllamaInstance("Thinker", "llm.int", 11434, ModelTier.THINKER, "nemotron-super", 131072),
    OllamaInstance("Writer",  "llm.int", 11435, ModelTier.WRITER,  "mistral-small",  32768),
    OllamaInstance("Analyst", "llm.int", 11436, ModelTier.ANALYST, "phi4",           16384, enabled=False),
]
...
# Reusing a single browser instance is much faster than launching per-URL
# and avoids the "browser leaks on error" problem.

_pw_browser = None          # playwright Browser instance
_pw_lock = asyncio.Lock()   # serialise launch/close
...
async def _get_browser():
    ...
    async with _pw_lock:
        if _pw_browser is not None:
            try:
                # Quick liveness check — if the browser crashed, contexts() throws
        ...
        log.info("Playwright Chromium browser launched (shared instance)")
        return _pw_browser
...
async def capture_screenshot(url: str) -> str:
    ...
    Strategy (in order):
      1. Playwright headless Chromium  (best — full render)
      2. httpx fetch → inline <img> embed of the raw response if it is an image
      3. httpx fetch → extract OG image / first <img> src
      4. SVG placeholder with real page title

    Results are cached by URL hash so each URL is only fetched once.
    Cache is NOT checked for SVG files so playwright can retry after a
    previous placeholder run.
    """
    key = hashlib.md5(url.encode()).hexdigest()[:16]
    png = SCREENSHOT_DIR / f"{key}.png"
    # Only skip if we already have a real screenshot
    if png.exists():
        return png.name
    ...
    try:
        ...
        try:
            ...
            await page.wait_for_timeout(600)   # let lazy images settle
            await page.screenshot(path=str(png), full_page=False, type="png")
            log.info("Screenshot OK (playwright): %s", url)
            return png.name
        except Exception as e:
            log.warning("Playwright page error for %s: %s", url, e)
        finally:
            await page.close()
    except ImportError:
        log.debug("playwright not installed — using fallback")
    except Exception as e:
        log.warning("Playwright browser error: %s — using fallback", e)
        global _pw_browser
        _pw_browser = None   # force re-launch next time

    # ── 2 & 3. httpx fetch — try to grab an OG/social image URL ──────────────
    og_image_url = None
    page_title    = url[:70]
    try:
        ...
        # OG image (best bet for a meaningful thumbnail)
        ...
        if og:
            candidate = og.group(1).strip()
            if candidate.startswith("//"):
                candidate = "https:" + candidate
            elif candidate.startswith("/"):
                parsed = urlparse(url)
                candidate = f"{parsed.scheme}://{parsed.netloc}{candidate}"
            og_image_url = candidate

    except Exception as e:
        log.debug("httpx fetch for %s: %s", url, e)

    # Try to download the OG image and save as PNG proxy
    if og_image_url:
        try:
            async with httpx.AsyncClient(timeout=8.0, follow_redirects=True) as c:
                r = await c.get(og_image_url,
                    headers={"User-Agent": "Mozilla/5.0 (compatible; Vera/1.0)"})
                ct = r.headers.get("content-type","")
                if ct.startswith("image/") and len(r.content) > 1000:
                    png.write_bytes(r.content)
                    log.info("Screenshot OK (og:image proxy): %s → %s", url, og_image_url)
                    return png.name
        except Exception as e:
            log.debug("OG image download failed %s: %s", og_image_url, e)

    # ── 4. SVG placeholder with real title ────────────────────────────────────
    svg = SCREENSHOT_DIR / f"{key}.svg"
    ...
    svg.write_text(f"""<svg xmlns="http://www.w3.org/2000/svg" width="640" height="380">
  ...
  <text x="320" y="350" font-family="monospace" font-size="9" fill="#c8c4bc" text-anchor="middle">{ex(url[:90])}</text>
    ...
    log.debug("Screenshot placeholder written for %s", url)
    return svg.name
...
async def deep_crawl_url(url: str, depth: int, breadth: int, timeout: float) -> str:
...
async def search_ddg(query: str, limit: int) -> list[dict]:
    try:
        ...
        return [{"url":u,"title":t,"content":snippets[i] if i<len(snippets) else ""}
...
async def gather_web_search(query: str, job_id: str) -> list[Citation]:
    ...
    for item in results:
        ...
        shot_tasks.append(capture_screenshot(url))
        if web_cfg.crawl_depth > 0:
            crawl_tasks.append(deep_crawl_url(url, web_cfg.crawl_depth,
...
async def gather_arxiv(query: str, limit: int = 5) -> list[Citation]:
    if not any(s.id=="arxiv" and s.enabled for s in sources): return []
    try:
        async with httpx.AsyncClient(timeout=10.0) as c:
...
async def gather_hackernews(query: str, limit: int = 4) -> list[Citation]:
    if not any(s.id=="hackernews" and s.enabled for s in sources): return []
    try:
...
async def gather_all_sources(query: str, job: ResearchJob) -> tuple[list[Citation], str]:
    ...
    if active & {"searxng","brave","crawl4ai"}:
...
ANALYST_TIMEOUT      = 180.0   # analyst verification max seconds
THINKER_PLAN_TIMEOUT = 180.0   # planning/decompose/outline steps (JSON responses)
THINKER_THINK_TIMEOUT= 360.0   # deep reasoning/synthesis steps (long prose)
WRITER_TIMEOUT       = 300.0   # writer section/extraction steps
SUMMARY_TIMEOUT      = 180.0   # rolling context summary update
...
async def run_single(job: ResearchJob, project: Optional[Project] = None):
    ...
    parts: list[str] = []
    async for tok in stream_ollama(writer,
        f"Research query: {job.query}{proj_ctx}\n\n{ctx}\n\nCitations:\n{cit_ref}\n\n"
...
async def run_parallel(job: ResearchJob, project: Optional[Project] = None):
    ...
    async def research_sub(q: str, idx: int) -> str:
        ...
        async for tok in stream_ollama(use_w,
            f"Sub-question: {q}\n\n{ctx}{proj_ctx}",
            "Answer this sub-question concisely and accurately.", job.id):
            parts.append(tok)
            if cancel_flags.get(job.id): break
        return "".join(parts)

    sub_results = await asyncio.gather(*[research_sub(q,i) for i,q in enumerate(sub_qs)])
    ...
    if saved_sources:
        ...
        for row in saved_sources:
            try:
                ...
                log.warning("Skipping saved source %s: %s", row.get("id"), e)
        log.info("Loaded %d sources from DB", len(sources))

    # ── Load persisted instances (if any) ─────────────────────────────────────
    ...
    if saved_insts:
        ...
        for row in saved_insts:
            try:
                ...
                log.warning("Skipping saved instance %s: %s", row.get("name"), e)
    ...
    for row in saved_projects:
        if row["id"] not in projects:
            proj = Project(
                ...
                output_mode=OutputMode(row.get("output_mode", "report")),
...
async def test_source(req: SourceTestRequest):
    ...
    if src.id=="searxng":
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as c:
                r=await c.get(host); ok=r.status_code<400; detail=f"HTTP {r.status_code}"
        except Exception as e: detail=str(e)
    elif src.type==SourceType.NEO4J:
        try:
            from neo4j import AsyncGraphDatabase  # type:ignore
            drv=AsyncGraphDatabase.driver(src.config["uri"],auth=(src.config.get("user","neo4j"),src.config.get("password","")))
            await drv.verify_connectivity(); await drv.close(); ok=True; detail="Connected"
        except ImportError: detail="neo4j not installed"
        except Exception as e: detail=str(e)
    elif src.type==SourceType.CHROMA:
        try:
            import chromadb  # type:ignore
            chromadb.HttpClient(host=src.config.get("host","llm.int"),port=int(src.config.get("port",8000))).heartbeat()
            ok=True; detail="Heartbeat OK"
        except ImportError: detail="chromadb not installed"
        except Exception as e: detail=str(e)
    elif src.type==SourceType.REDIS:
        try:
            import redis.asyncio as aioredis  # type:ignore
            r=aioredis.Redis(host=src.config.get("host","llm.int"),port=int(src.config.get("port",6379)),
                password=src.config.get("password") or None,db=int(src.config.get("db",0)),decode_responses=True)
            await r.ping(); await r.aclose(); ok=True; detail="PONG"
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as c:
                r=await c.get("https://api.search.brave.com/res/v1/web/search",
                    ...
                    headers={"Accept":"application/json","X-Subscription-Token":src.config.get("api_key","")})
...
async def _do_research(nb: dict, cell: dict, nb_ctx: str, query: str) -> str:
    ...
    ctx_str=""
    try:
        class _FJ: id="nb"; sources=[]; citations=[]
        res=await gather_web_search(query,_FJ())
        await _cell_broadcast(cid,{"type":"sources","count":len(res)})
        ctx_str="\n\n".join(f"[{i+1}] {c.title}\n{c.snippet[:300]}" for i,c in enumerate(res[:8]))
    except Exception as e: log.debug("nb research: %s",e)
    writer=await get_instance(ModelTier.WRITER) or await get_instance(ModelTier.THINKER)
    if not writer: await _cell_broadcast(cid,{"type":"error","text":"No model"}); return ""
    sys_p=("You are a research assistant writing for a notebook. Use sources and context. "
           "Cite as [1],[2] etc. Use ## headings and bullet points.")
    prompt=f"{nb_ctx}\n\nQuery: {query}\n\nSources:\n{ctx_str}\n\nWrite a comprehensive notebook section."
    parts=[]
    async for tok in stream_ollama(writer,prompt,sys_p,f"cell:{cid}",timeout_secs=WRITER_TIMEOUT):
        parts.append(tok); await _cell_broadcast(cid,{"type":"token","text":tok})
    generated="".join(parts)
...
if __name__ == "__main__":
    ...
    uvicorn.run(f"{module_name}:app", host="0.0.0.0", port=8766,
