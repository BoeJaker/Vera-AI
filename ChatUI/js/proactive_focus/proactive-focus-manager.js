(() => {
            ...
            <div id="focusControlBar"style="display: flex; flex-wrap: wrap; gap: 8px; justify-content: space-between; align-items: center; margin-bottom: 20px; padding: 16px; background: rgba(15, 23, 42, 0.5); border-radius: 8px; border: 1px solid #334155;">
                <div style="display: flex; flex-wrap: wrap; gap: 8px; align-items: center;">
                    ${this.currentFocus ? `
                        ...
                        <button class="focus-header-btn" onclick="app.triggerProactiveThought()">
                            Think Now
                        </button>
                    ` : ''}
                    <button class="focus-header-btn" onclick="app.showFocusBoardMenu()">
                        Load
                    </button>
                    <button class="focus-header-btn" onclick="app.loadFocusStatus()">
                        Refresh
                    </button>
                    <button class="focus-header-btn" onclick="app.saveFocusBoard()">
                        Save
                    </button>
                </div>
        ...
        html += `
            <div style="display: flex; flex-wrap: wrap; gap: 4px; margin-bottom: 0;">
                ${categories.map(cat => {
                    ...
                    const count = cat.key === 'actions' ? this._getExpandedActionsCount(items) : items.length;
                    return `
    ...
    VeraChat.prototype._getExpandedActionsCount = function(items) {
        let count = 0;
        items.forEach(item => {
    ...
    VeraChat.prototype.renderTabContent = function(cat) {
        ...
        const hasStageControl = ['ideas', 'next_steps', 'actions'].includes(cat.key);
        ...
        if (hasStageControl && this.currentFocus) {
            ...
            if (stageMap[cat.key]) {
                html += `<button class="panel-btn" style="font-size: 11px; padding: 6px 12px;" onclick="app.${stageMap[cat.key]}()">▶️ Generate</button>`;
            }
        }
        ...
        html += `
                        <button class="panel-btn" style="font-size: 11px; padding: 6px 12px;" onclick="app.showAddItemForm('${cat.key}')">+ Add</button>
                        ${items.length > 0 ? `<button class="panel-btn" style="font-size: 11px; padding: 6px 12px;" onclick="app.clearCategory('${cat.key}')">🗑️ Clear</button>` : ''}
                    </div>
                </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 10px;">
                ...
                <div style="display: flex; gap: 8px;">
                    <button class="panel-btn" onclick="app.submitActionForm('${category}')" 
                            style="flex: 1; font-size: 13px; padding: 10px;">✓ Add Action</button>
                    <button class="panel-btn" onclick="app.hideAddItemForm('${category}')" 
                            style="font-size: 13px; padding: 10px;">✕ Cancel</button>
                </div>
            </div>
        `;
    };
    ...
    VeraChat.prototype.renderActionItem = function(item, idx, color, category) {
        ...
        return `
            <div id="${itemId}" class="draggable-item" draggable="true" 
                ...
                ` : ''}
                <div style="display: flex; gap: 6px; flex-wrap: wrap;">
                    <button class="panel-btn" style="font-size: 11px; padding: 4px 8px;" onclick="app.executeActionDirectly(${idx})">▶️ Execute</button>
                    <button class="panel-btn" style="font-size: 11px; padding: 4px 8px;" onclick="app.editBoardItem('${category}', ${idx})">✏️ Edit</button>
                    <button class="panel-btn" style="font-size: 11px; padding: 4px 8px;" onclick="app.deleteBoardItem('${category}', ${idx})">🗑️</button>
                    <button class="panel-btn" style="font-size: 11px; padding: 4px 8px;" onclick="app.moveToCompleted('${category}', ${idx})">✓ Complete</button>
                </div>
            </div>
        `;
    };
    ...
    VeraChat.prototype.showFocusBoardSelectionModal = function(fileBoards, historyBoards) {
        ...
        let html = `
            ...
            <div id="fileBoardsTab" class="focus-board-tab">
        `;
        ...
        html += `</div><div id="historyBoardsTab" class="focus-board-tab" style="display: none;">`;
    ...
    VeraChat.prototype._renderSetFocusModal = function(similarFocuses) {
        ...
        let html = `
            ...
            <div id="similarFocusesContainer">
        `;
