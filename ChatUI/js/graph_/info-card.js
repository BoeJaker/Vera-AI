...
(function() {
    ...
    window.GraphInfoCard = {
        ...
        setupEventListeners: function() {
            ...
            network.on("selectNode", function(params) {
                if (params.nodes.length > 0 && !self.inlineMode) {
                    console.log('Node selected, expanding:', params.nodes[0]);
         ...
         */
        expandNodeInfo: function(nodeId) {
            ...
            this.renderExpandedView(nodeId, nodeData);
        },
        ...
        /**
         * Render expanded view - THEME COMPATIBLE
         */
        renderExpandedView: function(nodeId, nodeData) {
            const displayName = nodeData.display_name || nodeId;
            const labels = nodeData.labels || [];
            const properties = nodeData.properties || {};
            ...
            const vectorContent = this.renderVectorContent(nodeId);
            if (vectorContent) {
                html += `
                    <div style="margin-bottom: 16px;">
                        <div style="color: var(--text-secondary); font-size: 12px; font-weight: 600; margin-bottom: 8px;">Vector Content</div>
                        ${vectorContent}
                    </div>
                `;
            }
