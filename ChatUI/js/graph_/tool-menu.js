...
(function() {
    ...
    window.GraphToolExecutor = {
        ...
        selectItem: async function(nodeId, itemName, itemType) {
            ...
            try {
                ...
                const endpoint = itemType === 'plugin'
                    ...
                    : `${this.apiBaseUrl}/api/toolchain/${this.sessionId}/tool/${encodeURIComponent(itemName)}/schema`;
        ...
        showItemInputForm: function(nodeId, nodeName, itemName, itemType, schema) {
            ...
            if (nodeData && nodeData.properties && Object.keys(nodeData.properties).length > 0) {
                ...
                for (const [key, value] of Object.entries(nodeData.properties)) {
                    ...
                    propertiesHtml += `
                        <div 
                            draggable="true"
                            ondragstart="window.GraphToolExecutor.handlePropertyDragStart(event, '${this.escapeHtml(key)}', ${JSON.stringify(value).replace(/'/g, "&apos;")})"
                            style="
            ...
            schema.parameters.forEach((param, index) => {
                ...
                formHtml += `
                    ...
                    "
                    ondrop="window.GraphToolExecutor.handleFieldDrop(event, '${this.escapeHtml(param.name)}')"
         ...
         */
        handlePropertyDragStart: function(event, propName, propValue) {
            this.draggedProperty = { name: propName, value: propValue };
            event.dataTransfer.effectAllowed = 'copy';
            event.dataTransfer.setData('text/plain', String(propValue));
            event.target.style.opacity = '0.5';
            console.log('Dragging property:', propName, '=', propValue);
        },
         ...
         */
        handleFieldDrop: function(event, paramName) {
            event.preventDefault();
            event.currentTarget.style.borderColor = '#1e293b';
            event.currentTarget.style.background = '#0f172a';
            ...
            const inputField = document.getElementById(`param-${paramName}`);
            ...
            console.log('Dropped property', this.draggedProperty.name, 'into field', paramName);
        ...
        executeItem: async function(nodeId, itemName, itemType) {
            ...
            try {
                ...
                if (itemType === 'plugin') {
                    ...
                    response = await fetch(
                        `${this.apiBaseUrl}/api/toolchain/${this.sessionId}/execute-tool?tool_name=${encodeURIComponent(itemName)}&tool_input=${encodeURIComponent(JSON.stringify(itemInput))}`,
                        { method: 'POST' }
                    );
                }
                ...
                // Show result
                this.showItemResult(nodeId, itemName, itemType, itemInput, result.output, duration);
         ...
         */
        showItemResult: function(nodeId, itemName, itemType, input, output, duration) {
            if (!window.GraphInfoCard) {
                alert('GraphInfoCard not available');
                return;
            }
            ...
            const content = `
                ...
                <div style="display: flex; flex-direction: column; gap: 8px;">
                    <button onclick="window.GraphToolExecutor.saveResultToNode('${nodeId}', '${itemType}', ${JSON.stringify(output).replace(/'/g, "&apos;")})" style="
