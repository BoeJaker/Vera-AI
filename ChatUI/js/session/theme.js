...
renderRGB: {
  ...
  variables: {
    ...
    '--accent-r': '#ff2d2d',        // red block
    '--accent-g': '#2dff6a',        // green block
    '--accent-b': '#2d6aff',        // blue block
  ...
  css: `
    ...
    /* Remove headers/avatars (global) */
    .message-header, .message-avatar, .chat-message-header, .chat-message-avatar {
      display: none !important;
    }

    /* Chat message area: stack of inline lines (render style) */
    #chatMessages {
      display: flex;
    ...
    /* Each message is a single rendered line with ink line + block color behind name/prefix if needed */
    .message {
      display: flex;
    ...
    /* Ensure rendered message is inline so pseudo before sits on same baseline */
    .message-rendered {
      display: inline;
      color: var(--text);
      background: none;
    ...
    /* SYSTEM prefix: make it appear as the first char inline */
    ...
    /* USER prefix uses color blocks (small square) */
    .message.user::before {
      content: "";
      display: inline-block;
      width: 10px;
    ...
    /* ink-styled inline decoration for the rendered text */
    .message-rendered {
      position: relative;
      padding-left: 0;
      color: var(--text);
      -webkit-text-stroke: 0.25px rgba(0,0,0,0.5); /* subtle ink edge */
      text-shadow: 0 1px 0 rgba(0,0,0,0.45);
    }

    /* Render art: occasional block color behind short tags or labels */
    .message .tag {
      display:inline-block;
      padding: 2px 6px;
      border-radius: 4px;
    ...
    /* example tag color blocks (RGB theme) */
    .tag.r { background: var(--accent-r); }
    .tag.g { background: var(--accent-g); }
    .tag.b { background: var(--accent-b); }

    /* Faked ink stroke lines between messages (like hand separated blocks) */
    ...
    /* Input area - stylized like a drafting strip / terminal hybrid */
    .input-area {
      display: flex;
      align-items: center;
      gap: 8px;
    ...
    .send-btn {
      ...
      border-radius: 6px;
      font-weight: 600;
      box-shadow: 0 6px 18px rgba(45,106,255,0.12);
    }
    .send-btn:hover { transform: translateY(-1px); }

    /* small responsive collapse: when container is narrow show only eyes/head (we'll wire via JS) */
    .vera-collapsed svg { width: 40px; height: 40px; }
  ...
  graph: {
    nodeBorder: '#ff2d2d',
    nodeBackground: '#0f1113',
    nodeHighlight: '#2d6aff',
    nodeFont: '#e6eef8',
    nodeFontSize: 14,
    edgeColor: '#27313a',
    edgeHighlight: '#2d6aff',
    background: '#0b0b0c',
    fontFamily: '"Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'
  }
},

// B: Render - CMY palette
renderCMY: {
  name: 'Render — CMY',
  variables: {
    '--bg': '#080909',
    ...
    '--accent': '#00cfe8',   // cyan block
    '--accent-c': '#00cfe8',   // cyan block
    '--accent-m': '#ff46b0',   // magenta block
    '--accent-y': '#ffd100',   // yellow block
  ...
  css: `
    ...
    .chat-panel, .graph-panel {
      background: linear-gradient(180deg, rgba(255,255,255,0.01), transparent 30%), var(--panel-bg);
      border: 1px solid var(--border);
      padding: 12px;
      border-radius: 8px;
      box-shadow: 0 8px 36px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.02);
    }
    ...
    .message + .message::before {
      ...
      height:1px;
      margin:6px 0;
      background: linear-gradient(90deg, rgba(255,255,255,0.02), rgba(0,0,0,0.35));
      opacity: 0.28;
    }

    .input-area { display:flex; align-items:center; gap:8px; padding:10px; background: linear-gradient(90deg, rgba(255,255,255,0.01), transparent); border-top:1px solid rgba(255,255,255,0.02); }

    .input-wrapper { display:flex; align-items:center; gap:8px; flex:1; background: linear-gradient(180deg, rgba(255,255,255,0.006), transparent); border-radius:6px; padding:6px; border:1px dashed rgba(255,255,255,0.02); }
    ...
    .send-btn { background: var(--accent-m); border:none; color:white; padding:6px 10px; border-radius:6px; font-weight:700; box-shadow: 0 6px 18px rgba(255,70,176,0.12); }
    .send-btn:hover { transform: translateY(-1px); }

    .vera-collapsed svg { width:40px; height:40px; }

    .message-rendered::selection { background: rgba(0,207,232,0.14); color: var(--text); }

    .graph { background: var(--panel-bg); }
  `,
  graph: {
    nodeBorder: '#00cfe8',
    nodeBackground: '#0e0f10',
    nodeHighlight: '#ff46b0',
    nodeFont: '#f2f7fb',
    nodeFontSize: 14,
    edgeColor: '#202428',
    edgeHighlight: '#ff46b0',
    background: '#080909',
    fontFamily: '"Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'
  }
},
    ...
    terminal: {
      ...
      css: `
        ...
        /* Panels are just flat terminal windows */
        .chat-panel, .graph-panel {
          background: black;
          border: 1px solid var(--border);
          padding: 8px;
        ...
        /* MESSAGES LOOK LIKE TERMINAL LINES */
        #chatMessages {
          display: flex;
          flex-direction: column;
          padding: 0;
        ...
        /* Remove bubble spacing entirely */
        .message {
          display: block;
          padding: 0;
          margin: 0;
        ...
        /* Hide the header area above each message */
        .message-header {
            display: none !important;
        }

        /* Hide the avatar next to each message */
        .message-avatar {
            display: none !important;
        }

        /* Prefix styling for terminal look */
        .message.user {
          display: flex;
          align-items: center;
          flex-direction: row;
        ...
        // /* Make sure the rendered message stays inline with the before element */
        // .message.user .message-rendered {
        //   display: inline;        /* keeps everything on one line */
        // }
        ...
        /* Input bar looks like a CLI prompt */
        #messageInput {
          background: black;
          color: var(--text);
          border: none;
...
bidwellsDark: {
  ...
  variables: {
    ...
    '--panel-bg': '#0d1522',
    '--text': '#dbeafe',           /* soft pale blue text */
    '--text-secondary': '#93adc9',
    '--accent': '#1e3a8a',         /* Bidwells-style rich blue */
