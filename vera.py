...
class ProactiveFocusManager:
    """Enhanced with streaming support and WebSocket broadcasting"""
    ...
    def __init__(
        ...
        self._websockets = []  # For WebSocket connections
        self.current_thought = ""  # Track current streaming thought
    ...
    def set_focus(self, focus: str):
        ...
        self._broadcast_sync("focus_changed", {"focus": focus})
    ...
    def _run_proactive_loop(self):
        ...
        while self.running:
            ...
            if ollama_count >= self.max_ollama_processes:
                print(f"[FocusManager] High Ollama demand ({ollama_count} processes) — pausing...")
                self._broadcast_sync("proactive_paused", {"reason": "high_ollama_demand"})
                
                self.pause_event.clear()
                while self.running and self._count_ollama_processes() > self.max_ollama_processes:
                    time.sleep(2)
                
                print("[FocusManager] Ollama demand dropped — resuming...")
    ...
    def _generate_proactive_thought_streaming(self) -> Optional[str]:
        ...
        # Broadcast thought generation started
        ...
        try:
            ...
            # Broadcast completed thought
            self._broadcast_sync("thought_completed", {
                "thought": thought
            })
        ...
        try:
            ...
            if result:
                ...
                self._broadcast_sync("goal_execution_completed", {
                    "goal": goal,
                    "result": result[:500]  # Truncate for broadcast
                })
