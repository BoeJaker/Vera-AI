...
class LLMTool:
    ...
    def coding_llm_query(self, query: str) -> str:
        """
        Query a coding LLM for code generation, debugging, or code-related tasks.
        Best for: code generation, debugging, code review.
        Note: Optimized for code but may not perform as well on general language tasks.
        """
        try:
            result = ""
            use_orchestrator = hasattr(self.agent, 'orchestrator') and self.agent.orchestrator and self.agent.orchestrator.running
            ...
            # Store in memory
            self.agent.mem.add_session_memory(
                self.agent.sess.id, result, "Code",
                {"topic": "coding_llm", "agent": self.agent.selected_models.deep_llm}  # Changed to attribute access
            )
            return result
        except Exception as e:
            return f"[Deep LLM Error] {str(e)}"
